...
function concatFactory() {
  if (env.nodejs) {
    return (b1, b2) => {
      const buffer = Buffer.concat([Buffer.from(b1.buffer, b1.byteOffset, b1.length), Buffer.from(b2.buffer, b2.byteOffset, b2.length)]);
      return new Uint8Array(buffer.buffer, buffer.byteOffset, buffer.length);
    };
  }
...
function createJwsFactory() {
  if (env.nodejs) {
    return ({
      ...
      const buffer = Buffer.concat([Buffer.from(encodedHeader + '.', 'utf8'), Buffer.from(verifyData.buffer, verifyData.byteOffset, verifyData.length)]);
      return new Uint8Array(buffer.buffer, buffer.byteOffset, buffer.length);
    };
  }
